...
def from_robot_to_map_fr(robot_pose, person_pose):
    rx, ry, ra = robot_pose['x'], -robot_pose['y'], robot_pose['a']
    y, x, a = person_pose
    transform = np.array([[math.cos(-ra), -math.sin(-ra), rx],
                                        [math.sin(-ra), math.cos(-ra), ry],
                                        [0, 0, 1]])
    ...
    relative_pose = transform @ np.array([[x], [y], [1.]], dtype=object)
    x_person = relative_pose[0][0]
    y_person = relative_pose[1][0]
    new_pose = np.array([x_person, y_person, angle])
    return new_pose
...
for f in filenames:
    ...
    for sample in all_data:
        ...
        data = sample['people_poses']
        r_pose = sample["robot_pose"]
        # data.reverse()
        if not data:
            continue
        ...
        for i_f in range(len(data)):
            for i_p, p in enumerate(data[i_f]["people"]):
                p_pose = np.array([p['x'], p['y'], p['a']])
                new_pose = from_robot_to_map_fr(r_pose, p_pose)
                data[i_f]["people"][i_p]['x'] = new_pose[0]
                data[i_f]["people"][i_p]['y'] = new_pose[1]
                data[i_f]["people"][i_p]['a'] = new_pose[2]
            #     if i_p==0:
            #         data[i_f]["people"][i_p]['y'] -= 1.5
            #     else:
            #         data[i_f]["people"][i_p]['y'] += 1
            # if len(data[i_f]['people'])==2:
            #     data[i_f]['interaction'] = [{'dst': 1, 'src': 2, 'relation': 'interaction'}]
        ...
        time_2 = time.time()
        print("time graphs", time_1 - time_0, 'time model', time_2 - time_1, 'total', time_2-time_0)
